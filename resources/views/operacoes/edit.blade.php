            ...
            <div class="col-lg-8 mx-auto">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">Editar Operação</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('operacoes.update', $operacao->id) }}" method="POST" class="form-editar-operacao" data-no-loading>
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nome" class="form-control" 
                                       value="{{ old('nome', $operacao->nome) }}" required>
                                @error('nome')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="codigo" class="form-control" 
                                       value="{{ old('codigo', $operacao->codigo) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="descricao" class="form-control" rows="3">{{ old('descricao', $operacao->descricao) }}</textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" id="valor_aprovacao_automatica" name="valor_aprovacao_automatica" class="form-control" inputmode="decimal"
                                           data-mask-money="brl" placeholder="0,00" value="{{ old('valor_aprovacao_automatica', $operacao->valor_aprovacao_automatica) }}">
                                </div>
                                <small class="text-muted">
                                    ...
                                    <br><strong>Nota:</strong> Apenas novos empréstimos (criados após esta edição) serão afetados por esta configuração.
                                </small>
                            ...
                            @php
                                $docObrigatoriosAtuais = old('documentos_obrigatorios', $operacao->documentosObrigatorios->pluck('tipo_documento')->toArray());
                            @endphp
                            @foreach($tiposDocumento ?? \App\Modules\Core\Models\OperacaoDocumentoObrigatorio::tiposDisponiveis() as $chave => $label)
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" name="documentos_obrigatorios[]" 
                                           id="doc_obr_{{ $chave }}" value="{{ $chave }}"
                                           {{ in_array($chave, $docObrigatoriosAtuais) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="requer_aprovacao" id="requer_aprovacao" 
                                           value="1" {{ old('requer_aprovacao', $operacao->requer_aprovacao ?? true) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="requer_liberacao" id="requer_liberacao" 
                                           value="1" {{ old('requer_liberacao', $operacao->requer_liberacao ?? true) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="requer_autorizacao_pagamento_produto" id="requer_autorizacao_pagamento_produto"
                                           value="1" {{ old('requer_autorizacao_pagamento_produto', $operacao->requer_autorizacao_pagamento_produto ?? false) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="requer_autorizacao_pagamento_produto">
                                        <strong>Permitir pagamento em produto/objeto</strong>
                                    </label>
                                </div>
                                <small class="text-muted">
                                    Se marcado, consultores poderão registrar pagamentos em produto/objeto (não gera caixa). Esses pagamentos precisam ser aceitos por gestor ou administrador em Liberações.
                                </small>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="consultores_veem_apenas_proprios_emprestimos" class="form-select" id="visibilidade_emprestimos_consultores">
                                    <option value="0" {{ old('consultores_veem_apenas_proprios_emprestimos', $operacao->consultores_veem_apenas_proprios_emprestimos ?? false) ? '' : 'selected' }}>
                                        Todos da operação
                                    </option>
                                    <option value="1" {{ old('consultores_veem_apenas_proprios_emprestimos', $operacao->consultores_veem_apenas_proprios_emprestimos ?? false) ? 'selected' : '' }}>
                                        Apenas os empréstimos dele
                                    </option>
                                </select>
                                <small class="text-muted">
                                    <strong>Todos da operação:</strong> consultores veem na listagem todos os empréstimos da operação (incluindo de outros consultores).<br>
                                    <strong>Apenas os empréstimos dele:</strong> cada consultor vê somente os empréstimos em que ele é o responsável. Gestores e administradores continuam vendo todos.
                                </small>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="consultor_pode_vender" id="consultor_pode_vender"
                                           value="1" {{ old('consultor_pode_vender', $operacao->consultor_pode_vender ?? false) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="consultor_pode_vender">
                                        <strong>Consultor pode vender e ver produtos (somente leitura)</strong>
                                    </label>
                                </div>
                                <small class="text-muted">
                                    Se marcado, usuários com papel <strong>consultor</strong> nesta operação poderão registrar vendas e consultar o catálogo de produtos, sem cadastrar ou editar produtos.
                                </small>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" name="taxa_juros_atraso" class="form-control" 
                                           ...
                                           placeholder="0.00" 
                                           value="{{ old('taxa_juros_atraso', $operacao->taxa_juros_atraso ?? 0) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="tipo_calculo_juros" class="form-select">
                                    <option value="por_dia" {{ old('tipo_calculo_juros', $operacao->tipo_calculo_juros ?? 'por_dia') === 'por_dia' ? 'selected' : '' }}>
                                        Por Dia
                                    </option>
                                    <option value="por_mes" {{ old('tipo_calculo_juros', $operacao->tipo_calculo_juros ?? 'por_dia') === 'por_mes' ? 'selected' : '' }}>
                                        Por Mês
                                    </option>
                                </select>
                                <small class="text-muted">
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="ativo" 
                                           id="ativo" value="1" 
                                           {{ old('ativo', $operacao->ativo) ? 'checked' : '' }}>
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('operacoes.show', $operacao->id) }}" class="btn btn-secondary">
                                    <i class="bx bx-x"></i> Cancelar
                                </a>
                                <button type="submit" class="btn btn-primary">
                                    <i class="bx bx-check"></i> Atualizar Operação
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    @section('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.form-editar-operacao').forEach(form => {
                    const submitHandler = function(e) {
                        ...
                        Swal.fire({
                            ...
                            html: `Deseja salvar as alterações na operação <strong>${nome}</strong>?`,
                            icon: 'question',
